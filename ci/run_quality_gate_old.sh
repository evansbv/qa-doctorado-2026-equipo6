...
SYSTEMATIC_WEEK4="evidence/week4/systematic_summary.txt"

# Ejecutar script existente (ajusta path si es necesario)
if [ ! -f $SYSTEMATIC_SUMMARY ]; then
  echo "Copiando casos sistemáticos de week4 a week5..." 
  cp $SYSTEMATIC_WEEK4 "${SYSTEMATIC_SUMMARY}"
#else
#  echo "No se encontró el resumen de casos sistemáticos de week4. Se intentará ejecutar los casos directamente." | tee -a "${RUNLOG_FILE}"
#  scripts/systematic_cases.sh  2>&1 || true
#  cp $SYSTEMATIC_WEEK4 "${SYSTEMATIC_SUMMARY}"
fi
...
if grep -iq "FAIL" "${SYSTEMATIC_SUMMARY}"; then
  ...
  echo "Casos Sistemáticos" "FAIL" "Al menos un caso falló (ver systematic_summary.txt)"
  #exit 1
else
